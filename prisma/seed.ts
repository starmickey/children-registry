...
async function main() {
  ...
  const [
    ...
    parallelSeedAndMap(relationshipTypes, (data) =>
      prisma.relationshipType.create({ data }),
    ),
    parallelSeedAndMap(pins, (data) => prisma.pin.create({ data })),
    parallelSeedAndMap(permissionTypes, (data) =>
      prisma.permissionType.create({ data }),
    ),
    parallelSeedAndMap(diseases, (data) => prisma.disease.create({ data })),
  ]);
  ...
  const classIdMap = await parallelSeedAndMap(classes, (data) => {
    ...
    const mappedClassroomId = data.classroomId
      ? classroomIdMap.get(data.classroomId)
      : null;
  ...
  await Promise.all([
    ...
    (async () => {
      const validRegs = registrations.map((r) => {
        ...
        if (!childId || !classId)
          throw new Error("Invalid Registration relation");
    ...
    (async () => {
      const validPhones = phones.map(({ id, contactId, ...phone }) => {
        ...
        if (!mappedContactId)
          throw new Error(`Missing contactId for Phone ${id}`);
    ...
    (async () => {
      const validRels = relationships.map((r) => {
        ...
        if (!childId || !contactId || !relationshipTypeId)
          throw new Error("Invalid Relationship relation");
    ...
    (async () => {
      ...
      await prisma.pinGrant.createMany({
        data: validPinGrants,
        skipDuplicates: true,
      });
    })(),
    ...
    (async () => {
      const validPermissions = permissions.map((p) => {
        ...
        if (!childId || !permissionTypeId)
          throw new Error("Invalid Permission relation");
    ...
    // Disease assignations
    (async () => {
      const validDiseaseAssignations = diseaseAssignations.map((d) => {
        const childId = childIdMap.get(d.childId);
        const diseaseId = diseaseIdMap.get(d.diseaseId);
        if (!childId || !diseaseId)
          throw new Error("Invalid disease assignation relation");
        return { ...d, childId, diseaseId };
      });
      await prisma.diseaseAsignation.createMany({
        data: validDiseaseAssignations,
        skipDuplicates: true,
      });
    })(),
  ]);
